  ...
  return (
    <div className="p-4 space-y-4">
      <div className="grid gap-3 md:grid-cols-4">
        <RunCardStat label="架构版本" value={card.schema_version || "未知"} />
      ...
      <div className="grid gap-4 xl:grid-cols-2">
        ...
        <RunCardPanel title="验证" icon={ShieldCheck}>
          {card.validation ? (
            ...
            <p className="text-sm text-muted-foreground">无验证数据记录。</p>
          )}
        </RunCardPanel>
      </div>
      ...
      <RunCardPanel title="产物校验和" icon={FileCheck2}>
        {artifacts.length > 0 ? (
          ...
          <p className="text-sm text-muted-foreground">无产物校验和记录。</p>
        )}
      </RunCardPanel>
    </div>
  );
...
function TradesTab({ run }: { run: RunData }) {
  ...
  if (trades.length === 0) return <div className="p-8 text-muted-foreground text-sm">无成交记录。</div>;
  return (
    <div className="p-4">
      <table className="w-full text-sm">
        <thead>
